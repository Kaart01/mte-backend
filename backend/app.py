...
def push_to_airtable(variant_name, mte_value):
    ...
    try:
        requests.post(AIRTABLE_API_URL, headers=headers, json=data, timeout=10)
...
def get_variants(model_name):
    ...
    """, [model_name])
    return jsonify([{"variant_name": r["variant_name"], "MTE": float(r["MTE"])} for r in rows])
...
def calculate_mte():
    ...
    return jsonify({
        "overall_mte": total,
        "variants": [{"variant_name": r["variant_name"], "MTE": float(r["MTE"])} for r in rows]
    })
